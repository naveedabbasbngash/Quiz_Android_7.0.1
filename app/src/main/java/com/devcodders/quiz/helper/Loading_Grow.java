...
public class Loading_Grow extends View {
    public static final String TAG = Loading_Grow.class.getSimpleName();
    ...
    private ArrayList<Grow_Drawable> mDrawables = new ArrayList<>();

    public Loading_Grow(Context context) {
        this(context, null);
    }

    public Loading_Grow(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public Loading_Grow(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.GrowProgressbar);
        mNumberDots = a.getInt(R.styleable.GrowProgressbar_dd_numDots, 3);
        mDotRadius = a.getDimension(R.styleable.GrowProgressbar_android_radius, 8);
        mDotColor = a.getColor(R.styleable.GrowProgressbar_android_color, 0xff9c27b0);
        mDotEndColor = a.getColor(R.styleable.GrowProgressbar_dd_endColor, mDotColor);
        mDotScaleMultiplier = a.getFloat(R.styleable.GrowProgressbar_dd_scaleMultiplier, DEFAULT_GROWTH_MULTIPLIER);
        mAnimationDuration = a.getInt(R.styleable.GrowProgressbar_dd_animationDuration, 300);
        mHorizontalSpacing = a.getDimension(R.styleable.GrowProgressbar_dd_horizontalSpacing, 12);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        if (shouldAnimate()) {
            for (Grow_Drawable dot : mDrawables) {
                dot.draw(canvas);
            }
        }
    }
    ...
    private void initDots() {
        ...
        for (int i = 1; i <= mNumberDots; i++) {
            final Grow_Drawable dot = new Grow_Drawable(mDotColor, mDotRadius, mDotMaxRadius);
    ...
    private void updateDots() {
        ...
        for (int i = 0; i < mDrawables.size(); i++) {
            final Grow_Drawable dot = mDrawables.get(i);
